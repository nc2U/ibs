...
if [ -f "$SCRIPT_DIR/.env" ]; then
  ...
  if [ -e "$CURR_DIR/values-prod-custom.yaml" ]; then
    ...
    kubectl delete job -n ibs-prod -l app.kubernetes.io/name=web --ignore-not-found=true 2>/dev/null || true
    if kubectl get cluster postgres -n ibs-prod >/dev/null 2>&1; then
      ...
      kubectl get cluster postgres -n ibs-prod -o json | kubectl apply --server-side --force-conflicts --field-manager=helm -f - || true
    fi
