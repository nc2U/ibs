...
class SalesBillIssueForm(forms.Form):
    ...
    bank_number1 = forms.CharField(max_length=25, label='계좌번호[1]')
    bank_host1 = forms.CharField(max_length=20, label='예금주[1]')
    bank_account2 = forms.CharField(max_length=20, label='수납은행[2]', required=False)
    bank_number2 = forms.CharField(max_length=25, label='계좌번호[2]', required=False)
    bank_host2 = forms.CharField(max_length=20, label='예금주[2]', required=False)
    zipcode = forms.CharField(max_length=5, label='우편번호')
    address1 = forms.CharField(max_length=35, label='주소')
    address2 = forms.CharField(max_length=50, label='상세주소', required=False)
    address3 = forms.CharField(max_length=30, label='참고항목', required=False)
    title = forms.CharField(max_length=80, label='고지서 제목')
