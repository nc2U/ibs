"""
ProjectCashBook 분리 거래 데이터 검증 관리 명령어

사용법:
    python manage.py validate_cashbook_splits
    python manage.py validate_cashbook_splits --fix
    python manage.py validate_cashbook_splits --project=1
"""
...
class Command(BaseCommand):
    help = 'ProjectCashBook 분리 거래의 데이터 무결성 검증'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('ProjectCashBook 분리 거래 검증 시작'))
        self.stdout.write(self.style.SUCCESS('=' * 70))
        ...
        if options['project']:
            ...
            self.stdout.write(f"\n📌 프로젝트 ID {options['project']}만 검증합니다.\n")
