...
def get_ledger_standardized_payment_sum(project, date=None, date_range=None):
    ...
    if date_range:
        # ExportLedgerPaymentStatus용 날짜 범위
        queryset = queryset.filter(deal_date__range=date_range)
    elif date:
        # ExportLedgerPaymentStatus, ExportLedgerOverallSummary용 특정 날짜까지
        queryset = queryset.filter(deal_date__lte=date)

    return queryset.aggregate(total=Sum('accounting_entry__amount'))['total'] or 0


def get_ledger_standardized_payment_sum_by_order(project, date, installment_order_id):
    """
    Ledger 기반 회차별 표준화된 납부액 집계 (유효 계약자 입금만)
...
class ExportLedgerPaymentStatus(ExcelExportMixin, ProjectFilterMixin, AdvancedExcelMixin):
    """차수 및 타입별 수납 집계 현황 (Ledger 기반)"""
    ...
    def get(self, request):
        ...
        # ----------------- get_data_using_api start ----------------- #
        # ContractPaymentStatusByUnitTypeViewSet 사용 (Ledger 기반)
        from apiV1.views.payment import ContractPaymentStatusByUnitTypeViewSet
        ...
        # API ViewSet의 list 메서드 호출
        viewset = ContractPaymentStatusByUnitTypeViewSet()
        response = viewset.list(mock_request)
        api_data = response.data
        ...
        # 합계 계산 - 표준화된 집계 방식 사용 (Ledger 기반)
        standardized_paid_amount = get_ledger_standardized_payment_sum(project, date)
...
class ExportLedgerOverallSummary(ExcelExportMixin, ProjectFilterMixin, AdvancedExcelMixin):
    """총괄 집계 현황 (Ledger 기반)"""
    ...
    def get(self, request):
        ...
        from apiV1.views.payment import ContractPaymentOverallSummaryViewSet, ContractPaymentStatusByUnitTypeViewSet
        ...
        # API ViewSet의 list 메서드 호출
        viewset = ContractPaymentOverallSummaryViewSet()
        response = viewset.list(mock_request)
        api_data = response.data
        ...
        # ContractPaymentStatusByUnitTypeViewSet에서 금액 데이터 가져와서 총 계약률 계산
        payment_status_mock_request = Mock()
        payment_status_mock_request.query_params = {
            'project': str(project.pk),
            'date': date if date else 'null'
        ...
        # 수납액 - Ledger 기반 표준화된 집계 방식 사용 (회차별, 합계 모두 표준화)
        worksheet.write(row_num, 1, '수납액', center_format)
        standardized_collected_amount = get_ledger_standardized_payment_sum(project, date)
        # 회차별로도 표준화된 계산 사용
        for i, order in enumerate(pay_orders):
            standardized_order_collected = get_ledger_standardized_payment_sum_by_order(project, date, order['pk'])
            worksheet.write(row_num, 2 + i, standardized_order_collected, body_format)
        worksheet.write(row_num, col_count - 1, standardized_collected_amount, body_format)
        ...
        # 실수납액 - Ledger 기반 표준화된 집계 방식 사용 (회차별, 합계 모두 표준화)
        row_num += 1
        worksheet.write(row_num, 1, '실수납액', center_format)
        standardized_actual_collected = get_ledger_standardized_payment_sum(project, date)
        # 회차별로도 표준화된 계산 사용 (할인료, 연체료 미구현으로 수납액과 동일)
        for i, order in enumerate(pay_orders):
            standardized_order_actual = get_ledger_standardized_payment_sum_by_order(project, date, order['pk'])
            worksheet.write(row_num, 2 + i, standardized_order_actual, body_format)
        worksheet.write(row_num, col_count - 1, standardized_actual_collected, body_format)

        # 수납율 - Ledger 기반 표준화된 집계 기반으로 계산 (회차별, 합계 모두 표준화)
        row_num += 1
        worksheet.write(row_num, 1, '수납율', center_format)
        total_collection_rate = (
                standardized_actual_collected / total_contract_amount * 100) if total_contract_amount > 0 else 0
