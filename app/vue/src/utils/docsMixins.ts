...
const securityLevel = computed(() => docStore.docs?.security_level ?? '3')
...
export const docsManageItems = computed(() => [
  ...
  { title: securityTitle.value, icon: securityIcon.value },
  {
...
const toChangeSecurityLevel = (docs: number, state: boolean, filter: DocsFilter) =>
  patchDocs({
    pk: docs,
    security_level: state ? '3' : '1',
    filter,
  }).then(() =>
    message(
      'info',
      '',
      `이 문서를 ${state ? '3등급 (프로젝트 공개)' : '1등급 (비공개)'}로 변경하였습니다.`,
    ),
  )
...
export const toDocsManage = (fn: number, payload: ManagePayload) => {
  ...
  if (fn === 4) return toChangeSecurityLevel(docs, state, filter)
