...
<template>
  <CNav v-if="!route.path.startsWith('/manage') || workManager?.value" variant="tabs" class="mb-0 pl-4">
    <CDropdown v-if="route.params['projId']">
      ...
      <CDropdownMenu>
        ...
        <CDropdownItem v-if="workManager?.value" @click="router.push({ name: '(설정) - 범주추가' })">
          새 업무 범주
        </CDropdownItem>
        <CDropdownItem v-if="workManager?.value" @click="router.push({ name: '(로드맵) - 추가' })">
          새 버전
        </CDropdownItem>
        <CDropdownItem v-if="workManager?.value" @click="router.push({ name: '(소요시간) - 추가' })">
          작업시간 기록
        </CDropdownItem>
        <CDropdownItem v-if="workManager?.value" disabled>새 뉴스</CDropdownItem>
        <CDropdownItem v-if="workManager?.value" disabled>새 문서</CDropdownItem>
      </CDropdownMenu>
    </CDropdown>
