...
const currentPath = computed<string[]>(() => pathMatch.value.slice(0, -1))
...
onBeforeMount(async () => {
  const path = pathMatch.value.join('/')
  fileData.value = await fetchFileView(repoId.value, path, sha.value)
  await highlightCode()
})
</script>
