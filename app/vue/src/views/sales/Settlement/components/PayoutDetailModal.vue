      ...
      <CModalBody v-if="payout">
        ...
        <div class="bg-light p-3 rounded mb-3 border">
          <div class="d-flex justify-content-between align-items-center mb-2">
            <div>
              <span class="fs-5 fw-bold text-dark mr-2">{{ payout.sales_person_name }}</span>
          ...
          <div class="small text-secondary">
            ...
            입금 계좌: <strong>{{ payout.bank_name || '-' }}</strong> {{ payout.account_number }}
        ...
        <div class="row g-2 mb-3 text-center">
          <div class="col-4">
            <div class="border rounded p-2 bg-body-secondary">
              ...
              <div class="fw-bold text-primary fs-6">{{ payout.commission_amount.toLocaleString() }}원</div>
          ...
          <div class="col-4">
            <div class="border rounded p-2 bg-body-secondary">
              ...
              <div class="fw-bold text-dark fs-6">{{ payout.base_pay.toLocaleString() }}원</div>
            </div>
          </div>
          <div class="col-4">
            <div class="border rounded p-2 bg-body-secondary">
              <div class="small text-muted">공제/환수액 (차감)</div>
              <div class="fw-bold text-danger fs-6">-{{ payout.deduction_amount.toLocaleString() }}원</div>
            </div>
          </div>
        </div>
        ...
        <table class="table table-bordered small text-center mb-3">
          <thead class="table-light">
            <tr>
              ...
              <th>지방소득세 (0.3%)</th>
              <th>원천징수 합계 (3.3%)</th>
              <th class="table-primary text-primary fw-bold">실지급액 (세후)</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              ...
              <td class="text-danger font-monospace">{{ payout.local_income_tax.toLocaleString() }}원</td>
              <td class="text-danger fw-bold font-monospace">{{ payout.total_tax.toLocaleString() }}원</td>
              <td class="table-primary text-primary fw-bold fs-6 font-monospace">
            ...
            <table class="table table-sm table-hover table-bordered small text-center mb-0">
              ...
              <tbody>
                <tr v-for="c in payout.contract_details" :key="c.id">
                  <td class="font-monospace text-left">{{ c.contract_serial }}</td>
                  <td class="fw-bold">{{ c.contractor_name }}</td>
                  <td>
                    <span class="badge bg-light text-dark border">{{ c.role_type_display }}</span>
                  </td>
