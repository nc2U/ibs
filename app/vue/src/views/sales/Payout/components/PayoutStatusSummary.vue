...
const paidPayouts = computed(() => props.payouts.filter(p => p.pay_status === '3'))

const paidNetAmount = computed(() =>
  paidPayouts.value.reduce((sum, p) => sum + (p.net_amount || 0), 0),
)

const unpaidNetAmount = computed(() => Math.max(0, totalNetAmount.value - paidNetAmount.value))
...
const totalTaxAmount = computed(() => props.payouts.reduce((sum, p) => sum + (p.total_tax || 0), 0))
</script>
  ...
  <CRow class="mb-4 g-3">
    ...
    <CCol sm="6" lg="3">
      <CCard
        ...
        :class="unpaidNetAmount > 0 ? 'border-start-warning' : 'border-start-secondary'"
      >
        <CCardBody>
          <div class="text-body-secondary small fw-semibold">지급 잔액 (대기/보류)</div>
          <div
            class="fs-4 fw-bold"
            :class="unpaidNetAmount > 0 ? 'text-warning' : 'text-secondary'"
          >
