...
const batchUpdateStatus = async (targetStatus: string, label: string) => {
  ...
  try {
    const promises = selectedPayoutIds.value.map(id =>
      salesStore.updatePayStatus(id, targetStatus)
    )
...
const downloadBankingCsv = () => {
  const targets = selectedPayoutIds.value.length > 0
  ...
  const periodTitle = selectedPeriod.value?.title ? selectedPeriod.value.title.replace(/\s+/g, '_') : '수수료정산'
  ...
  <ContentBody>
    <CCardBody class="p-3 p-md-4 pb-5">
      ...
      <p class="mb-0 text-secondary">
        상단 프로젝트 선택 메뉴에서 관리하실 프로젝트를 선택하시면 수수료 지급 대장을 조회할 수 있습니다.
      </p>
    </div>
    ...
    <div v-else>
          ...
          <CRow class="g-3 align-items-center">
            ...
            <CCol md="4" lg="3">
              <div class="d-flex align-items-center gap-2">
                <span class="fw-bold text-dark text-nowrap small">
            ...
            <CCol md="4" lg="3">
              <div class="d-flex align-items-center gap-2">
                <span class="fw-bold text-dark text-nowrap small">지급 상태:</span>
      ...
      <CCard v-if="selectedPeriod" class="shadow-sm mb-4">
        <CCardHeader class="bg-light py-2 d-flex flex-wrap justify-content-between align-items-center gap-2">
          <!-- 좌측: 다중 선택 및 일괄 변경 버튼 -->
          <div class="d-flex align-items-center gap-2">
