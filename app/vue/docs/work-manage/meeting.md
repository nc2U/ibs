...
```mermaid
flowchart LR
    A["🎙️ AI 음성 녹음·업로드<br>Gemini 자동 구조화"] -.-> B["1. 회의 사전 준비<br>의제·참석자 등록"]
    B --> C["2. 회의 진행 & 기록<br>내용·결정사항·후속조치"]
    C --> D["3. 액션아이템 연동<br>업무(Issue) 발행 & 배정"]
    D --> E["4. 진척도 실시간 추적<br>완료율 프로그레스 바"]
    E --> F["5. 회의록 공식 확정<br>PDF 보관 & 활동 로그"]
```
...
회의록 입력 화면은 좌측의 **상세 본문 입력창**과 우측의 **메타 정보 설정창**으로 분리되어 직관적인 입력을 지원하며, 회의 템플릿 및 **AI 음성 회의록 자동 생성** 기능을 제공합니다.
...
### 3.3 🎙️ AI 음성 회의록 생성 (Gemini 멀티모달)

회의 등록 화면 상단의 **`[🎙️ AI 음성 회의록 생성]`** 버튼을 클릭하여 회의 음성으로부터 회의록 본문과 조치과제를 원클릭으로 자동 생성할 수 있습니다.

* **두 가지 입력 방식 지원**:
  * **실시간 현장 녹음**: 브라우저 마이크를 통해 회의 현장에서 직접 녹음(`[녹음 시작]` $\rightarrow$ 타이머 카운트 $\rightarrow$ `[녹음 완료 및 분석]`)
  * **기존 음성 파일 업로드**: 스마트폰이나 녹음기로 사전 녹음된 오디오 파일(`.m4a`, `.mp3`, `.wav`, `.webm`) 선택 업로드
* **Google Gemini 오디오 분석 파이프라인**:
  * 음성 인식(STT)과 지능형 문맥 분석을 결합하여 환각(Hallucination) 없이 실제 발화된 사실(Fact)만을 추출합니다.
  * 회의 내용에 가장 적합한 **회의 제목, 카테고리, 사전 안건, 본문 마크다운, 주요 결정사항, 후속 조치 과제**를 자동 구조화합니다.
* **입력 폼 자동 바인딩 & 액션아이템 자동 파싱**:
  * 추출된 데이터는 회의록 폼의 각 입력란에 즉시 자동으로 채워집니다.
  * 후속 조치(`action_items`)는 `- [ ] 조치내용 (담당: 담당자 / 기한: YYYY-MM-DD)` 양식으로 추출되어, 시스템의 **인라인 액션아이템 테이블로 즉시 자동 변환**되므로 담당자 배정 및 기한 연동이 원클릭으로 완료됩니다.
* **데이터 보안 및 프라이버시 (Zero Retention)**:
  * 회의록 구조화 데이터 추출이 완료되는 즉시 서버 및 Gemini API 상의 원본 음성 파일은 **영구 삭제**되므로 사내 중요 의사결정과 음성 데이터가 안전하게 보호됩니다.

> [!TIP]
> **회의 등록 시 후속 조치 업무 일괄 자동 발행**:
> 회의록 작성 시 후속 조치 본문에 액션아이템이 기재되어 있다면(수동 입력 또는 AI 자동 추출), 하단의 **`[회의 등록 시 후속 조치 업무 일괄 자동 발행]`** 체크박스가 활성화됩니다. 이를 체크하고 회의를 저장하면 회의록 생성과 동시에 각각의 후속 조치가 해당 워크스페이스의 [업무(Issue)](/work-manage/issue)로 자동 일괄 등록 및 연계됩니다.
