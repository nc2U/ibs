...
- **메뉴 위치**: `대외 공문 관리 > 발송 공문 관리` (`/official-letter/outbound`)
...
### 발신 명의 및 인장(직인) 선택
- **전자결재 상신 발송**: 결재선 승인 완료 시 공문서 하단 결재란에 `담당 [기안자] → [검토자] → [직책] [최종승인권자]`가 표준 형식으로 자동 배치되며, 최종 승인권자 바로 위에 승인/전결 일자가 표기됩니다. (대표이사 단독 기안 시 '담당' 중복 없이 대표이사 단독 표기)
- **수동(직접) 발송**: 전자결재 없이 직접 발송하는 공문으로, **기안/담당자명** 및 **담당 직위**를 입력합니다. (하단 결재란에 `담당 [기안자]`와 우측의 대표이사 서명이 배치됩니다)
- **날인 인감(직인) 선택**:
