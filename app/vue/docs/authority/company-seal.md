...
| **인장 종류** | `seal_type` | • `CORP_SEAL`: 법인인감 (대표이사 직인)<br>• `USAGE_SEAL`: 사용인감 (계약/분양 전용 등)<br>• `DEPT_SEAL`: 부서인감 / 현장소장 직인<br>• `SIGN`: 대표자 / 현장소장 전자 서명<br>• `OMIT`: 직인생략 |
| **인장 명칭** | `name` | 사용자가 쉽게 식별할 수 있는 명칭 (예: `[판교현장] 현장소장인`, `대표이사 법인인감`) |
| **인장 이미지** | `seal_image` | **배경이 투명한 PNG 형식(정방형 비율)** 권장 (PDF 인쇄 시 문서 배경과 자연스럽게 합성됨) |
| **관리 책임자/부서**| `manager` | 해당 실물 인장을 보관/관리하는 책임자 (예: `판교복합개발현장 현장소장 홍길동`) |
...
| **사용 여부** | `is_active` | 폐기되거나 분실/재발급된 인장은 체크 해제하여 사용 중단 처리 |
