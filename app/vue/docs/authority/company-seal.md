...
| **연동 전결 결재선** | `route_template` | **🔒 필수 안전장치**: 부서장/현장 직인(`DEPT_SEAL`)을 전자결재 공문에 사용하려면, 해당 책임자가 최종 전결권자로 지정된 수동 결재선 템플릿을 연동해야 합니다. 미연동 시 전자결재 발송 인장 목록에서 자동 제외됩니다. |
...
### 2) 현장소장 직인 / 부서장 직인 (`DEPT_SEAL`) 등록 시 (안전장치 ⭐️)
- **사내 총괄 관리책임자(`internal_manager`)** 및 **연동 결재선(`route_template`)** 설정 필수.
- **전자결재 발송 시**: 해당 책임자가 전결권자인 결재선 템플릿이 연결된 인장만 기안 화면에 표시되며, 상신 시 해당 전결 결재선이 자동으로 적용됩니다.
- **단독/직접 발송 시**: 인장 선택 시 등록된 책임자의 직책과 성명(예: `본부장 유용식`)이 발송자 명의에 고정 반영되어 권한 없는 자의 명의 도용을 원천 차단합니다.
