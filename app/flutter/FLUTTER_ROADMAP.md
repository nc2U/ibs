...
- [x] **Step 12**: Phase 3-2 - **모바일 전자결재 (Approval Core) 시스템 전면 구축 완료**
  ...
  - **기안자 보직(Staff Assignment) 검증 및 보안 격리**:
    - 임직원(Staff) 및 보직 부재 시 기안 작성 버튼(FAB) 숨김, 기안 화면 내 임시저장/상신 버튼 `disabled` 처리 및 안내 뷰 표출
    - 백엔드 `ApprovalDocumentSerializer.validate`에서 보직 미등록 사용자의 결재 생성 요청 원천 차단
...
- [x] **Step 11**: Phase 3-1 - **실시간 워크스페이스 메신저 (`Real-Time Chat & Direct Message`) 시스템 구축 완료**
  - 웹소켓(`WebSocket`) 기반 0.1초 실시간 송수신 및 자동 재연결
  - 1:1 다이렉트 메시지(DM) 및 워크스페이스 공용 대화방 지원
  - 이미지 및 첨부파일 업로드, 답장(Reply), 링크 미리보기, 미확인 메시지 배지 동기화
  - 백그라운드 FCM 푸시 알림 연동

---

## 🚀 5. 메뉴별 완성도 평가 및 추후 고도화 추천 로드맵

### 1) 📋 업무 관리 (Work Core) — 완성도: **98%** (Production-Ready)
* **현황**: 회의록(Meeting)과 업무(Issue) 간 유기적 연결, 진척률(0~100%), 첨부파일, 댓글, PDF 내보내기 및 **3대 뷰 모드(목록 뷰 ↔ 월간 캘린더 뷰 ↔ 간이 간트 타임라인 뷰)** 전면 완비.
* **추후 고도화 추천 사항**:
  1. **오프라인 퍼스트 동기화 (Offline Caching)**: 현장 음영지역에서 작성한 업무/회의록을 로컬 DB(Hive)에 임시 보관 후, 온라인 복귀 시 자동 백그라운드 동기화.
  2. **음성 메모 / AI 회의록 요약**: 모바일 마이크 녹음 파일을 백엔드 AI 전사 엔진과 연동하여 의제/결정사항 자동 초안 생성.

### 2) 🏗️ 프로젝트 관리 (Project Core) — 완성도: **90%**
* **현황**: 부동산 개발 사업지 전용 계약/수납/자금/문서/부지/설정 5대 도메인의 직각 미니멀(`radius=0`) 조회 및 고지서 공유 완비.
* **추후 고도화 추천 사항**:
  1. **모바일 현장 간편 수납/지출 등록**: 현장에서 영수증을 촬영하여 즉시 입출금 전표 또는 수납 영수증 초안을 생성하는 OCR 연동.
  2. **동호수 배치도(배치 매트릭스) 시각화 인터랙션**: 층/호수 그리드 터치 시 해당 유닛의 계약자/수납 상태 팝업 표시.
  3. **캐시플로우 미니 차트**: 월별 수납 목표 대비 실적 및 지출 추이를 모바일 미니 막대/꺾은선 차트로 시각화.

### 3) ✍️ 전자결재 (Approval Core) — 완성도: **98%** (Production-Ready)
* **현황**: 17종 전 양식 기안/상세 뷰, 결재선 타임라인, 원터치 승인/반려/회수, WeasyPrint PDF 네이티브 뷰어 및 임직원(Staff) 보직 기반 철통 권한 방어 완비.
* **추후 고도화 추천 사항**:
  1. **다건 일괄 승인 (Batch Action)**: 대표이사/임원 사용자를 위한 대기 목록 체크박스 다중 선택 및 일괄 승인 기능.
  2. **생체 인증 결재 확인 (Biometric Sign)**: 결재 승인 시 Face ID / 지문 인식을 통한 2차 본인 확인 옵션 연동.
  3. **자주 쓰는 결재선 즐겨찾기**: 수동 결재선 지정 시 최근/자주 사용하는 결재선 템플릿 불러오기.

### 4) 🏢 채널 & 전사 라운지 (Channel & Corporate Lounge) — 완성도: **92%**
* **현황**: 공지/게시판 피드, 전사 라운지(사명/가치관/조직도/사규/온보딩/FAQ/기술지원), 실시간 웹소켓 메신저(DM/그룹챗/파일첨부) 완비.
* **추후 고도화 추천 사항**:
  1. **사내 간이 투표 (Poll)**: 공지 및 게시글에 투표/설문 기능 추가.
  2. **중요 공지 상단 고정 (Pinned Post)**: 필독 공지 상단 배너 고정 및 읽음 확인(Check-read) 추적.
  3. **메신저 메시지 검색**: 대화방 내 키워드 검색 및 날짜별 메시지 점프 기능.

---

## 🎨 6. 테마 시스템(Theme) 아키텍처 & 개발 시 유의사항
...
| **공지사항 (News)** | **신규 공지 등록 (일반 / 긴급)** | 워크스페이스 소속 멤버 전원 (본사/공개 공지는 전체 사용자) | `[공지] {워크스페이스}` / `[긴급 공지]` | `"{공지제목}" 새 공지가 등록되었습니다.` | `send_news_push_task` |
| **게시판 (Forum)** | 신규 게시글 및 댓글 등록 | 워크스페이스 참여 멤버 (채널 피드 실시간 스트림 반영 완료) | `[게시글] {워크스페이스}` | 채널 피드 실시간 스트림 반영 중 | - |
...
*마지막 업데이트: 2026-08-31 (전자결재 기안자 보직 검증 및 권한 보안 격리, 실시간 메신저 완비, 4대 메뉴별 완성도 평가 및 추후 고도화 로드맵 반영)*