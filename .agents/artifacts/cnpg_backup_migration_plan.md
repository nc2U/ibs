...
cnpg:
  backup:
    ...
    s3:
      ...
      secretAccessKey: "access-secret-password"
```

**핵심 문제:**

- `backup.enabled: false` + `backup.s3.enabled: true` **불일치 상태**
- `cluster.yaml` L151: `{{- if and .Values.backup.enabled .Values.backup.s3.enabled }}` 조건으로 인해 실제 barmanObjectStore 및
  ScheduledBackup이 **전혀 생성되지 않는 상태**
